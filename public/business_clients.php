...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create_business_client') {
        ...
        // Validaciones
        $errors = [];
        if (!$personal_dni) $errors[] = "DNI es requerido";
        if (!$personal_first_name) $errors[] = "Nombres personales son requeridos";
        if (!$personal_last_name) $errors[] = "Apellidos personales son requeridos";
        if (!$business_id) $errors[] = "Identificador empresarial es requerido";
        if (!$business_name) $errors[] = "Nombre de empresa es requerido";
        if (!$business_type_id) $errors[] = "Tipo de empresa es requerido";
        if (!$business_position) $errors[] = "Cargo en la empresa es requerido";
        if ($user_id <= 0) $errors[] = "Usuario asociado es requerido";
        ...
        <div class="form-section">
            ...
            <div class="form-grid two">
                <div class="input-group">
                    ...
                    <input class="input" type="text" name="personal_dni" placeholder="Número de DNI" required maxlength="20">
                </div>
                <div class="input-group">
                ...
                <div class="input-group">
                    ...
                    <input class="input" type="text" name="personal_first_name" placeholder="Nombres" required maxlength="100">
                </div>
                <div class="input-group">
                    <label class="input-label">Apellidos *</label>
                    <input class="input" type="text" name="personal_last_name" placeholder="Apellidos" required maxlength="100">
                </div>
                <div class="input-group">
                    <label class="input-label">Email Personal</label>
                    <input class="input" type="email" name="personal_email" placeholder="correo@personal.com" maxlength="150">
                </div>
                <div class="input-group">
                ...
                <div class="input-group">
                    ...
                    <input class="input" type="text" name="personal_phone" placeholder="Número de teléfono" maxlength="30">
                </div>
            </div>
        </div>
        ...
        <div class="form-section" style="margin-top: 20px;">
            ...
            <div class="form-grid two">
                <div class="input-group">
                    ...
                    <input class="input" type="text" name="business_id" placeholder="RUC, NIT, etc." required maxlength="50">
                </div>
                <div class="input-group">
                    <label class="input-label">Nombre de la Empresa *</label>
                    <input class="input" type="text" name="business_name" placeholder="Nombre legal de la empresa" required maxlength="200">
                </div>
                <div class="input-group">
                    <label class="input-label">Teléfono Empresarial</label>
                    <input class="input" type="text" name="business_phone" placeholder="Teléfono de oficina" maxlength="30">
                </div>
                <div class="input-group">
                    <label class="input-label">Email Empresarial</label>
                    <input class="input" type="email" name="business_email" placeholder="correo@empresa.com" maxlength="150">
                </div>
                <div class="input-group">
                ...
                <div class="input-group">
                    ...
                    <input class="input" type="text" name="business_position" placeholder="Ej: Gerente, Propietario, etc." required maxlength="100">
                </div>
            </div>
            <div class="input-group" style="margin-top: 8px;">
                <label class="input-label">Dirección de la Empresa</label>
                <textarea class="input" name="business_address" rows="3" placeholder="Dirección completa de la empresa" maxlength="500"></textarea>
            </div>
        </div>
                                ...
                                <div style="margin-top:8px; padding:12px; background:var(--bg-alt); border-radius:8px;">
                                    ...
                                    <form method="post">
                                        ...
                                        <div style="display: grid; gap: 6px;">
                                            <input class="input" type="text" name="personal_dni" value="<?= h($client['personal_dni']) ?>" required>
                                            <input class="input" type="text" name="personal_first_name" value="<?= h($client['personal_first_name']) ?>" required>
                                            <input class="input" type="text" name="personal_last_name" value="<?= h($client['personal_last_name']) ?>" required>
                                            <input class="input" type="email" name="personal_email" value="<?= h($client['personal_email']) ?>">
                                            <select class="input" name="personal_gender" required>
                                            ...
                                            <input class="input" type="text" name="personal_phone" value="<?= h($client['personal_phone']) ?>">
                                            
                                            <input class="input" type="text" name="business_id" value="<?= h($client['business_id']) ?>" required>
                                            <input class="input" type="text" name="business_name" value="<?= h($client['business_name']) ?>" required>
                                            <input class="input" type="text" name="business_phone" value="<?= h($client['business_phone']) ?>">
                                            <input class="input" type="email" name="business_email" value="<?= h($client['business_email']) ?>">
                                            <select class="input" name="business_type_id" required>
                                                <?php foreach($business_types as $type): ?>
                                                    <option value="<?= h($type['id']) ?>" <?= $client['business_type_id'] == $type['id'] ? 'selected' : '' ?>>
                                                        <?= h($type['name']) ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                            <input class="input" type="text" name="business_position" value="<?= h($client['business_position']) ?>" required>
                                            <textarea class="input" name="business_address" rows="2"><?= h($client['business_address']) ?></textarea>
                                        </div>
                                        ...
                                        <div style="display:flex; gap:6px; margin-top:8px; flex-wrap:wrap;">
                                            ...
                                            <form method="post" style="display:inline;">
                                                ...
                                                <button type="submit" class="button <?= $client['status'] === 'active' ? 'warning' : 'success' ?> small">
