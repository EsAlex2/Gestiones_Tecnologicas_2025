...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'create_user' && is_admin()) {
    ...
    if ($id <= 0) redirect_with("/users.php", "ID inválido", "danger");
              ...
              <div style="margin-top:8px; padding:10px; background:var(--bg-alt); border-radius:8px;">
                ...
                <form method="post" style="display:grid; gap:6px;">
                  ...
                  <select class="input" name="role">
                    <option value="<?=ROLE_CLIENT?>" <?=$user['role']===ROLE_CLIENT?'selected':''?>>Cliente</option>
                    <option value="<?=ROLE_OPERATOR?>" <?=$user['role']===ROLE_OPERATOR?'selected':''?>>Operador</option>
                    <option value="<?=ROLE_ADMIN?>" <?=$user['role']===ROLE_ADMIN?'selected':''?>>Administrador</option>
                  </select>
                  <?php else: ?>
                ...
                <form method="post" id="passwordForm<?=h($user['id'])?>" style="display:none; margin-top:10px;">
                  ...
                  <input type="hidden" name="id" value="<?=h($user['id'])?>">
                  <input class="input" type="password" name="new_password" placeholder="Nueva contraseña" required minlength="8">
                  <div style="margin-top:6px;">
                    <button type="submit" class="button">Actualizar Contraseña</button>
                    <button type="button" class="button ghost" onclick="hidePasswordForm(<?=h($user['id'])?>)">Cancelar</button>
                  </div>
                </form>
              </div>
            </details>
